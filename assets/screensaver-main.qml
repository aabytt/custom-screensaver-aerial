...
WebOSWindow {
	...
	function checkStatus(){
		...
		if (videoOutput.status == MediaPlayer.EndOfMedia) playRandomVideo()
		if( videoOutput.status === 1) var status = 'NoMedia'
			else if(videoOutput.status === 2) var status = 'Loading'
			else if(videoOutput.status === 3) var status = 'Loaded'
			else if(videoOutput.status === 4) var status = 'Buffering'
			...
			else if(videoOutput.status === 6) var status = 'Buffered'
			else if(videoOutput.status === 7) var status = 'EndOfMedia'
			else if(videoOutput.status === 8) var status = 'InvalidMedia'
			else if(videoOutput.status === 0) var status = 'UnknownStatus'
			
		if(videoOutput.playbackState === 1) var playbackState = 'playing'
			else if(videoOutput.playbackState === 2) var playbackState = 'paused'
			else if(videoOutput.playbackState === 0) var playbackState = 'stopped'			 
		...
		debug.text = "Video " + randomIndex + " of " + playList.assets.length + 
				...
				"\n Buffer Progress : " + (videoOutput.bufferProgress*33.334).toFixed(0) + "%"
	}
	
	function updateOSD(){
		var DateFmt = ilib.require("DateFmt.js");
		var now = new Date();
		var time = new DateFmt({locale:settings.localeLang, timezone:"local", type:"time", length:"full"});
		var day = new DateFmt({locale:settings.localeLang, timezone:"local", type:"date", date:"dmw", length:"full"});
		timeOSD.text = time.format(now); 
		if(poi.date) dateOSD.text = poi.date.daysOfWeek[now.getDay()] + ", " + now.getDate() + " " + poi.date.months[now.getMonth()]
			else dateOSD.text = day.format(now);
			
		if (playList.assets[randomIndex].pointsOfInterest[Math.floor(videoOutput.position/1000)]) poiIndex = Math.floor(videoOutput.position/1000)		
	}
